...
void createScript()
{
    ofstream HootAHK("hoot shuffle debug.ahk");
    string str = "process, exist, hoot shuffle debug.exe ; prevents script from running unless hoot shuffle is running\n\
if !errorlevel\n\
    ExitApp\n\
process, exist, hoot.exe ; closes hoot shuffle if hoot was closed\n\
if !errorlevel\n\
    Process, Close, hoot shuffle debug.exe\n\
...
{\n\
    Loop\n\
    {\n\
        process, exist, hoot shuffle debug.exe ; closes the script if hoot shuffle gets closed\n\
        if !errorlevel\n\
            ExitApp\n\
...
int main()
{
    ...
    Sleep(2000);
    ShellExecute(NULL, "Open", "hoot shuffle debug.ahk", NULL, NULL, SW_SHOWNORMAL);
    Sleep(1000);
    cout << "Done!\n\n";
            ...
            for (UINT32 d = 0; d < nDevices; d++)
            {
                ...
                for (int session = 0; session < count; session++)
                {
                    ...
                    if (pwc != NULL)
                    {
                        ...
                        cout << "\nValue of peak_session is: " << peak_session << "\n";
                        cout << "Value of timesRepeated is: " << timesRepeated << "\n";
                        if (peak_session > 0 && timesRepeated < 8)
                        {
                            cout << "You should only see this message if peak_session is greater than 0 and timesRepeated is less than 8." << "\n";
                            float currentPeakValue = peak_session;
                            if (currentPeakValue == lastPeakValue)
                            {
                                cout << "Audio repeated! incrementing timesRepeated by 1." << "\n";
                                timesRepeated++; // need this to catch some buggy songs that hang on a note but aren't playing anything
                            }
                            else
                            {
                                cout << "Resetting timesRepeated and countToShuffle to 0.\n";
                                cout << "Value of timesRepeated before reset: " << timesRepeated << "\nValue of countToShuffle before reset: " << countToShuffle << endl;
                        ...
                        {
                            cout << "timesRepeated has reached the threshold to set countToShuffle to 8. Shuffle should happen next." << "\n";
                            countToShuffle = 8;
                        }
                        else
                        {
                            countToShuffle++;
                            cout << "No audio detected! Incremented countToShuffle. Its value is: " << countToShuffle << "\n";
                        }
